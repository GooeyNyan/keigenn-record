...
type IRemoveOverlayListener = <T extends EventType>(
  event: T,
  cb: EventMap[T],
) => void;
...
let queue:
  | (
      ...
      | [
          { [s: string]: unknown },
          ((value: string | null) => unknown) | undefined,
        ]
    )[]
  | null = [];
let rseqCounter = 0;
...
const sendMessage = (
  ...
  if (ws) {
    ...
    if (queue) queue.push([msg, cb]);
    else window.OverlayPluginApi.callHandler(JSON.stringify(msg), cb);
  }
};

const processEvent = <T extends EventType>(
  msg: Parameters<EventMap[T]>[0],
): void => {
  init();
...
export const startOverlayEvents: () => void = () => {};

export const removeOverlayListener: IRemoveOverlayListener = (
  event,
  cb,
): void => {
  init();
...
type OverrideMap = {
  [call in OverlayHandlerTypes]?: OverlayHandlerFuncs[call];
};
...
export const callOverlayHandler: IOverlayHandler = (
  ...
  const callFunc =
    callOverlayHandlerOverrideMap[type] ?? callOverlayHandlerInternal;
...
export const init = (): void => {
  ...
  if (typeof window !== "undefined") {
      ...
      const connectWs = function (wsUrl: string) {
        ...
        ws.addEventListener("message", (_msg) => {
          try {
            ...
            const promiseFuncs =
              msg?.rseq !== undefined ? responsePromises[msg.rseq] : undefined;
            if (msg.rseq !== undefined && promiseFuncs) {
              if (msg["$error"]) promiseFuncs.reject(msg);
              else promiseFuncs.resolve(msg);
    ...
    window.startOverlayEvents = startOverlayEvents;
  }
