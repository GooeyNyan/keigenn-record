...
const combatantMemoryKeys: readonly Extract<
  keyof PluginCombatantState,
  string
>[] = [
  "CurrentWorldID",
  "WorldID",
...
export type RepeatingFieldsTypes = keyof {
  [type in LogDefinitionTypes as LogDefinitions[type] extends RepeatingFieldsNarrowingType
    ? type
    : never]: null;
};
...
export type RepeatingFieldsDefinitions = {
  [type in RepeatingFieldsTypes]: LogDefinitions[type] & {
    readonly repeatingFields: Exclude<
      LogDefinitions[type]["repeatingFields"],
      undefined
    >;
  };
};
